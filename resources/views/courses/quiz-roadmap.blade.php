...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .step-link.disabled { pointer-events: none; cursor: default; }
        .step .node {
        ...
        .step.completed .node strong { color: #5f4309; }
        .step.locked .node {
            opacity: 0.38;
            filter: saturate(0.55);
            box-shadow: 0 16px 26px rgba(5, 10, 20, 0.12);
        }
        .step.locked .meta { opacity: 0.48; }
...
<div class="page">
    <div class="topbar">
        ...
        <div class="top-actions">
            <a class="link" href="{{ route('courses.quiz.info', ['quiz' => $course->id]) }}">{{ __('ui.course.course_info') }}</a>
            ...
            <a class="link" href="{{ route('courses.quiz.info', ['quiz' => $course->id]) }}#review">{{ __('ui.course.review') }}</a>
...
<script>
    ...
    async function updateConsistentMode(enabled) {
        ...
        try {
            const response = await fetch('{{ route('courses.consistent-mode.update', 'quiz-'.$course->id) }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                    'X-Requested-With': 'XMLHttpRequest',
